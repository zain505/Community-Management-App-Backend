import type { ApiError, ApiSuccess, ManagedUserStatus } from '@community/contracts';
...
function throwAuthServiceUnavailable(details: Record<string, unknown>): never {
  logger.error(details, 'Failed to verify managed user status with auth-service');
...
export const authClient = {
  async getManagedUserStatus(userId: string): Promise<ManagedUserStatus | null> {
    const url = `${env.AUTH_SERVICE_BASE_URL}/internal/auth/users/${encodeURIComponent(userId)}/managed-status`;

    try {
      const response = await axios.get<ManagedUserStatusResponse>(url, {
